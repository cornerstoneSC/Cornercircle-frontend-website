...
const fieldClass="mt-2 w-full rounded-md border border-stone-300 bg-white px-3 py-2.5 text-sm text-stone-800 focus:border-[#52704d] focus:outline-none focus:ring-2 focus:ring-[#52704d]/20";
...
export default function CurrentServicesEditor(){
  ...
  function field(section:PageKey,key:string,label:string,multiline=false){const value=String(content[section][key as keyof typeof content[typeof section]]);return <label className="block text-sm font-semibold text-stone-700">{label}{multiline?<textarea rows={key==="title"?3:2} className={fieldClass} value={value} onChange={event=>edit(section,key,event.target.value)}/>:<input className={fieldClass} value={value} onChange={event=>edit(section,key,event.target.value)}/>}</label>}
  ...
  return <section className="border-b border-stone-200 bg-[#ebe8df]">
    ...
    <div className="mx-auto grid max-w-[1700px] gap-5 p-5 xl:grid-cols-[350px_minmax(0,1fr)] xl:p-8">
      <aside className="self-start rounded-xl border border-stone-200 bg-[#fbfaf7] p-5 shadow-sm xl:sticky xl:top-28"><div className="mb-5 flex items-center gap-2 border-b border-stone-200 pb-4"><FilePenLine size={18} className="text-[#52704d]"/><div><h2 className="font-serif text-2xl text-[#173b2c]">Edit {pages.find(page=>page.key===active)?.label}</h2><p className="text-xs text-stone-500">Changes appear in the preview immediately.</p></div></div>{loading?<p className="text-sm text-stone-500">Loading content…</p>:<div className="grid gap-4">{field(active,"eyebrow","Eyebrow")}{field(active,"title","Hero title",true)}{field(active,"description","Hero description",true)}{active!=="main"&&<>{field(active,"primaryCta","Primary button")}{field(active,"secondaryCta","Secondary link")}</>}{active==="companionship"&&<>{field("companionship","hourlyRate","Hourly rate")}{field("companionship","minimum","Minimum booking")}{field("companionship","startingTotal","Starting total")}<div className="border-t border-stone-200 pt-4"><p className="mb-2 text-sm font-semibold text-stone-700">Who companionship is for</p><div className="grid gap-2">{content.companionship.audience.map((item,index)=><textarea rows={2} key={index} aria-label={`Audience item ${index+1}`} className={fieldClass} value={item} onChange={event=>editAudience(index,event.target.value)}/>)}</div></div></>}</div>}{error&&<p role="alert" className="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-800">{error}</p>}{message&&<p role="status" className="mt-4 rounded-md bg-green-50 p-3 text-sm text-green-800">{message}</p>}</aside>
