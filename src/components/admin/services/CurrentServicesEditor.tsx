...
export default function CurrentServicesEditor(){
  ...
  const [loading,setLoading]=useState(true); const [ready,setReady]=useState(false); const [busy,setBusy]=useState(false); const [message,setMessage]=useState(""); const [error,setError]=useState("");
  ...
  useEffect(()=>{getCurrentServicesContent().then(value=>{setContent(value);setSaved(value);setReady(true)}).catch(err=>setError(`${err instanceof Error?err.message:"Unable to load Services content."} Publishing is disabled until the saved content loads.`)).finally(()=>setLoading(false));return()=>{Object.values(pendingPhotos.current).forEach(photo=>URL.revokeObjectURL(photo.preview))}},[]);
