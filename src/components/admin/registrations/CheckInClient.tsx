...
export default function CheckInClient({ initialTicket }: { initialTicket: string }) {
  ...
  return <section className="min-h-full bg-[var(--event-canvas)] p-5 text-[var(--event-text)] md:p-10"><div className="mx-auto max-w-2xl">
    ...
    <form onSubmit={submit} className="mt-7 flex flex-col gap-2 sm:flex-row"><label className="flex min-h-12 flex-1 items-center gap-3 rounded-[var(--event-radius-sm)] border border-[var(--event-border)] bg-[var(--event-surface)] px-4 focus-within:border-[var(--event-accent-strong)] focus-within:ring-2 focus-within:ring-[var(--event-accent-soft)]"><ScanLine className="h-5 w-5 shrink-0 text-[var(--event-accent-strong)]" /><input className="min-w-0 w-full bg-transparent outline-none" value={ticket} onChange={(e) => setTicket(e.target.value)} placeholder="Paste ticket code or scanned URL" aria-label="Ticket code" /></label><button disabled={busy || !ticket.trim()} className="min-h-12 rounded-[var(--event-radius-sm)] bg-[var(--event-accent-strong)] px-6 font-semibold text-white disabled:opacity-50">Validate</button></form>
