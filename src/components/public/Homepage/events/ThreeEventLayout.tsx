...
interface ThreeEventLayoutProps { events: Event[]; }
...
export default function ThreeEventLayout({ events }: ThreeEventLayoutProps) {
  return (
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {events.slice(0, 3).map((event, index) => {
            ...
            return (
              <article key={event.id} className="group overflow-hidden border border-[var(--event-border)] bg-[var(--event-canvas)]">
                <div className="relative aspect-[16/10] overflow-hidden bg-[var(--event-accent-soft)]">
                  {event.coverImageUrl ? (
                    <Image
                ...
                <div className="p-5">
                  ...
                  <h3 className="mt-2 font-serif text-[22px] font-normal leading-tight text-[var(--event-heading)]">{event.title}</h3>
                  <div className="mt-4 space-y-2 text-xs leading-5 text-[var(--event-text)]">
                  ...
                  <Link href={`/events/${event.slug}`} className="mt-5 inline-flex items-center gap-2 text-xs font-medium text-[var(--event-heading)] transition-colors hover:text-[var(--event-accent)]">
