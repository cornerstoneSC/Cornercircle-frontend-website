...
export default function CompanionshipDetailsPage({ googleBookingUrl, content }: { googleBookingUrl?: string; content: CurrentServicesContent }) {
  return <div className={styles.page}>
    <CompanionshipHero content={content.companionship}/>
    ...
    <section className={styles.audiencePricing} aria-labelledby="audience-heading">
      ...
      <div className={styles.audienceCopy}><p className={styles.eyebrow}>Personalized support</p><h2 id="audience-heading">Who companionship<br/>is for</h2><p>Our companionship services are designed for:</p><ul>{content.companionship.audience.map(item => <li key={item}><Check aria-hidden="true" />{item}</li>)}</ul><blockquote>Life is brighter when it’s shared.</blockquote></div>
      <article className={styles.premiumPricing}><p className={styles.eyebrow}>Simple, transparent pricing</p><h2>{content.companionship.hourlyRate} <small>/ hour</small></h2><hr/><strong>{content.companionship.minimum}</strong><span>{content.companionship.startingTotal}</span><hr/><p>Recurring and extended visits can be discussed during your consultation.</p><a href="#consultation">{content.companionship.primaryCta} <span aria-hidden="true">→</span></a><small>No obligation. Just a conversation.</small></article>
    </section>
