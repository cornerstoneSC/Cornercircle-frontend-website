...
async function forward(request: Request, context: { params: Promise<{ segments?: string[] }> }) {
  ...
  const allowed = segments.length === 0 || segments.length === 1 && uuid.test(segments[0]) || segments.length === 2 && uuid.test(segments[0]) && ["renewal-reminder", "notes"].includes(segments[1]);
